...
- 发布包继续保持离线优先：模型/运行时准备好后，录音识别和文件转写不依赖外网。`ffmpeg.exe` 和 `ffprobe.exe` 已随安装包内置，普通用户不需要手动配置 ffmpeg。

详见：[Hi-Voicer 1.1.1 发布说明](docs/release-1.1.1.md)

## 下载与安装
从 [cg202601/Hi-Voicer Releases](https://github.com/cg202601/Hi-Voicer/releases) 下载最新版本：

- 推荐普通用户使用 `Hi-Voicer_1.1.1_x64-setup.exe`
- 也可以下载同版本 MSI 安装包

首次配置模型时需要联网下载模型和 Sherpa-ONNX CPU 运行时。配置完成后，录音识别、文件转写和基础音频处理在本地运行。正式发布包内置 `ffmpeg.exe` 和 `ffprobe.exe`，用于音频转码、字幕片段导出、双轨混音、波形生成和媒体信息探测；不内置 `ffplay.exe`。

## 当前模型策略

1.1.1 keeps Sherpa-ONNX CPU as the default stable path for ordinary Windows machines. CUDA product entry points have been removed; DirectML is available as a per-machine acceleration path after the Diagnostics CPU vs DirectML benchmark passes.
...
```powershell
gh attestation verify .\Hi-Voicer_1.1.1_x64-setup.exe --repo cg202601/Hi-Voicer
```
